...
# Adding JAVA_OPTS for the AppDynamics appserver agent.
JVM_OPTS="$JVM_OPTS -javaagent:/opt/appdyn/appserveragent/javaagent.jar -Dappdynamics.agent.accountAccessKey=153bcb0f2fee -Dappdynamics.agent.accountName=choicetest -Dappdynamics.agent.applicationName=ChoiceEdge -Dappdynamics.agent.nodeName=$HOSTNAME -Dappdynamics.agent.tierName=lt_ce_inventory -Dappdynamics.controller.hostName=choicetest.saas.appdynamics.com -Dappdynamics.controller.port=443 -Dappdynamics.controller.ssl.enabled=True"
...
#JVM_OPTS="$JVM_OPTS -XX:MaxTenuringThreshold=1" # Per CASSANDRA-8150 Jira; see GC Tuning below
